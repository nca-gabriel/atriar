...
function generateRpmData() {
    ...
    const totalRequests = [1500, 1800, 2200, 2500, 2900, 3100, 3400, 3000, 2800, 2500, 2000, 1900];
    // Success is slightly less than total
    const successRequests = totalRequests.map(r => Math.floor(r * (0.95 + (Math.random() * 0.03))));
    // Errors are a small fraction of the total
    const errorRequests = totalRequests.map(r => Math.floor(r * (0.05 - (Math.random() * 0.02))));
    // Example endpoint traffic
    const userEndpoint = [500, 600, 750, 850, 950, 1050, 1150, 1000, 900, 800, 650, 600];

    return { labels, totalRequests, successRequests, errorRequests, userEndpoint };
}

// Generates mock log entries for the Recent Requests table
function generateMockLogData(count = 15) {
    const methods = ['GET', 'POST', 'PUT', 'DELETE'];
    const paths = ['/api/v1/users', '/api/v1/data', '/api/v1/todos', '/api/v1/auth', '/api/v1/products'];
    ...
    for (let i = 0; i < count; i++) {
        ...
        const duration = status >= 500 ? (Math.random() * 500 + 150).toFixed(0) : (Math.random() * 100 + 50).toFixed(0);
...
function createSparkline(canvasId, data, color) {
    ...
    let fillColor;
    switch(color) {
        case '#6ee67b': fillColor = 'rgba(110, 230, 123, 0.15)'; break; // Uptime/Success (Green)
        case '#ffaa33': fillColor = 'rgba(255, 170, 51, 0.15)'; break; // Latency/Warning (Orange)
        case '#ff6e6e': fillColor = 'rgba(255, 110, 110, 0.15)'; break; // Error (Red)
        default: fillColor = 'rgba(255, 255, 255, 0.1)';
    }
...
function initRequestsPerMinuteChart() {
    const { labels, totalRequests, successRequests, errorRequests, userEndpoint } = generateRpmData();
...
function initFrequentEndpointsChart() {
    const ctx = document.getElementById('frequentEndpointsChart').getContext('2d');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    createSparkline('uptimeSparkline', generateMockSparklineData(99.8, 0.2), '#6ee67b'); 
    createSparkline('latencySparkline', generateMockSparklineData(150, 50), '#ffaa33');
    createSparkline('errorRateSparkline', generateMockSparklineData(0.2, 0.2, 30), '#ff6e6e');
